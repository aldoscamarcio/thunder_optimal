...
// - gradient of tau wrt ddq - //
Eigen::MatrixXd thunder_franka::get_gradddq(){
	Eigen::MatrixXd out;
	out.resize(7,7);
	long long p3[franka_gradddq_fun_SZ_IW];
	double p4[franka_gradddq_fun_SZ_W];
	const double* input_[] = {q.data(), dq.data(), ddq.data()};
	double* output_[] = {out.data()};
	int check = franka_gradddq_fun(input_, output_, p3, p4, 0);
	return out;
}

// - gradient of tau wrt dq - //
Eigen::MatrixXd thunder_franka::get_graddq(){
	Eigen::MatrixXd out;
	out.resize(7,7);
	long long p3[franka_graddq_fun_SZ_IW];
	double p4[franka_graddq_fun_SZ_W];
	const double* input_[] = {q.data(), dq.data(), ddq.data()};
	double* output_[] = {out.data()};
	int check = franka_graddq_fun(input_, output_, p3, p4, 0);
	return out;
}

// - gradient of tau wrt q - //
Eigen::MatrixXd thunder_franka::get_gradq(){
	Eigen::MatrixXd out;
	out.resize(7,7);
	long long p3[franka_gradq_fun_SZ_IW];
	double p4[franka_gradq_fun_SZ_W];
	const double* input_[] = {q.data(), dq.data(), ddq.data()};
	double* output_[] = {out.data()};
	int check = franka_gradq_fun(input_, output_, p3, p4, 0);
	return out;
}
